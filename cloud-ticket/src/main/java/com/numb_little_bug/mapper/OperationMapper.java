...
public interface OperationMapper {
    ...
    @Select("insert into operation (id, type, itemId, description, notice, stepNumber, ticketId) values (#{id}, #{type}, #{itemId}, #{description}, #{notice}, #{stepNumber}, #{ticketId})")
    void addOperation(Operation operation);

    @Select("delete from operation where id = #{id}")
    void deleteOperation(Integer id);

    @Select("update operation set type = #{type}, itemId = #{itemId}, description = #{description}, notice = #{notice}, stepNumber = #{stepNumber}, ticketId = #{ticketId} where id = #{id}")
